...
<script type="text/javascript">
	...
	function startRequest(){
		...
		xhr.open("get", "removeFile.do?newfilename=${bvo.newfilename}",true);
		xhr.send(null);
	}
	function callback(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				document.getElementById("updateView").innerHTML = 
		...
		<td>
			<table width="550">
				...
				<tr>
					...
					<span id="updateView">
						<c:choose>
							<c:when test="${bvo.orgfilename != null}">
								${bvo.orgfilename} <a href="javascript:startRequest()">파일삭제</a>
							</c:when>
							<c:otherwise>
								<input type="file" name="uploadFile">
							</c:otherwise>
